...
const adminContainer = document.querySelector('.admin-container') as HTMLDivElement;

// Sidebar controls
const menuToggle = document.getElementById('menuToggle') as HTMLButtonElement;
const sidebar = document.getElementById('sidebar') as HTMLElement;
const overlay = document.getElementById('overlay') as HTMLDivElement;
const closeSidebar = document.getElementById('closeSidebar') as HTMLButtonElement;
...
try {
	...
	if (Array.isArray(existing) && existing.length && existing.every((o: any) => demoIds.has(String(o.id)))) {
...
function showUsers() {
	...
	Array.from(document.querySelectorAll('[data-action="delete-user"]')).forEach((btn) => {
		btn.addEventListener('click', (event) => {
			...
			const email = (btn as HTMLButtonElement).getAttribute('data-email');
...
function showProducts() {
	...
	addProductBtn.addEventListener('click', () => {
		...
		const description = (document.getElementById('productDescription') as HTMLTextAreaElement).value.trim();
		const rating = Number((document.getElementById('productRating') as HTMLInputElement).value);
		const category = (document.getElementById('productCategory') as HTMLInputElement).value.trim();
	...
	saveChangesBtn.addEventListener('click', () => {
		...
		if (rating < 0 || rating > 5) { Toast.warning('Rating must be between 0 and 5.'); return; }
		if (!title || !price || !image || !description || !category) { Toast.warning('Please fill all required fields.'); return; }

		let all = JSON.parse(localStorage.getItem('products') || '[]');
		all = all.map((p: any) => p.id === editProductId ? { ...p, title, price, image, description, rating, category } : p);
		localStorage.setItem('products', JSON.stringify(all));
		editProductId = null;
		nameInput.value = '';
		(priceInput as any).value = '';
		imageInput.value = '';
		(document.getElementById('productDescription') as HTMLTextAreaElement).value = '';
		(document.getElementById('productRating') as HTMLInputElement).value = '' as any;
	...
	searchBar.addEventListener('input', () => {
		...
		if (query === '') { productsList.innerHTML = renderProducts(products); attachProductEvents(); return; }
		const filtered = products.filter((p: any) => p.title.toLowerCase().includes(query));
		...
		Array.from(document.querySelectorAll('#productsList button')).forEach((btn) => {
			btn.addEventListener('click', (event) => {
				...
				if (action === 'delete') {
					...
					if (product) {
						(document.getElementById('productName') as HTMLInputElement).value = product.title;
						(document.getElementById('productPrice') as HTMLInputElement).value = String(product.price);
						(document.getElementById('productImage') as HTMLInputElement).value = product.image;
						editProductId = id!;
						(document.getElementById('addProductBtn') as HTMLButtonElement).classList.add('hidden');
						(document.getElementById('saveChangesBtn') as HTMLButtonElement).classList.remove('hidden');
					}
				}
			});
		});
...
function showProductInfo(productId: string | null) {
	if (!adminContainer) return;
	const products = JSON.parse(localStorage.getItem('products') || '[]');
...
if (AuthManager.isAuthenticated() && AuthManager.getCurrentUser()?.email === ADMIN_EMAIL) {
	showDashboard();
	(document.getElementById('dashboardBtn') as HTMLButtonElement).addEventListener('click', (e) => { e.preventDefault(); if (adminContainer) adminContainer.innerHTML = ''; showDashboard(); });
	(document.getElementById('usersBtn') as HTMLButtonElement).addEventListener('click', (e) => { e.preventDefault(); if (adminContainer) adminContainer.innerHTML = ''; showUsers(); });
	(document.getElementById('productsBtn') as HTMLButtonElement).addEventListener('click', (e) => { e.preventDefault(); if (adminContainer) adminContainer.innerHTML = ''; showProducts(); });
	(document.getElementById('ordersBtn') as HTMLButtonElement).addEventListener('click', (e) => { e.preventDefault(); if (adminContainer) adminContainer.innerHTML = ''; showOrders(); });
} else {
	if (adminContainer) adminContainer.innerHTML = '<div class="p-8 bg-white rounded-xl shadow"><h3 class="text-xl font-medium text-red-600">Access Denied</h3><p class="mt-2">You do not have admin privileges.</p></div>';
}
